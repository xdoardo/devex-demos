...
pub trait CHERIoTCapability {
    fn make_deeply_immutable(&mut self);
}

impl<T> CHERIoTCapability for &T {
    fn make_deeply_immutable(&mut self) {
        unsafe {
            // The Rust compiler will eventually generate this for you.
            *self = core::mem::transmute(core::intrinsics::cheri::cheri_permissions_and((*self) as *const T as *const (), 0b111111110011))
        }
    }
}
